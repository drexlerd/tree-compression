...
/// Found some code on this: https://github.com/DaanWoltgens/ClearyCuckooParallel/tree/main
    ...
    public:
        ...
        bool exists(uint32_t slot) const {
            return slot_to_index.find(slot) != slot_to_index.end();
        }

        size_t size() const {
            return index_to_slot.size();
        }
    ...
    private:
        absl::flat_hash_map<uint32_t, uint32_t> slot_to_index;
