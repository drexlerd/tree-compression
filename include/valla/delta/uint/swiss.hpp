...
/// @brief Recursively insert the delta encoded elements from `it` until `end` into the `table`.
...
inline Index insert_recursively(Iterator it, Iterator end, size_t size, IndexedHashSet<Index>& table, Index& prev)
{
    /* Base cases */
    if (size == 1)
    {
    ...
    if (size == 2)
    {
        ...
        prev = i2;
        return table.insert(Slot<Index>(left_delta, right_delta));
    }
...
/// @param table is the table to uniquely insert the slots.
/// @return the slot representing the state.
template<std::ranges::input_range Range>
    requires std::same_as<std::ranges::range_value_t<Range>, Index>
auto insert(const Range& state, IndexedHashSet<Index>& table)
{
    ...
    auto prev = Index(0);
    return Slot<Index>(insert_recursively(state.begin(), state.end(), size, table, prev), size);
}
...
inline void read_state_recursively(Index index, size_t size, const IndexedHashSet<Index>& table, IndexList& ref_state, Index& prev)
{
    /* Base case */
    if (size == 1)
    {
    ...
    /* Conquer */
    read_state_recursively(slot.i1, mid, table, ref_state, prev);
    read_state_recursively(slot.i2, size - mid, table, ref_state, prev);
}

/// @brief Read the `out_state` from the given `tree_index` from the `table`.
/// @param index
/// @param size
/// @param table
/// @param out_state
inline void read_state(Index tree_index, size_t size, const IndexedHashSet<Index>& table, IndexList& out_state)
{
    out_state.clear();
    ...
    auto prev = Index(0);
    read_state_recursively(tree_index, size, table, out_state, prev);
}
...
/// @param out_state is the output state.
inline void read_state(Slot<Index> root_slot, const IndexedHashSet<Index>& table, IndexList& out_state)
{
...
public:
    ...
    const_iterator(const IndexedHashSet<Index>& table, Slot<Index> root, bool begin) : m_table(&table), m_stack(), m_value(END_POS)
    {
        if (begin)
        {
