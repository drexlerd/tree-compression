...
#ifndef VALLA_INCLUDE_PLAIN_DOUBLE_HASH_ID_MAP_HPP_
#define VALLA_INCLUDE_PLAIN_DOUBLE_HASH_ID_MAP_HPP_
...
namespace valla::plain::dbl::hash_id_map
{
...
inline Index
insert_recursively(Iterator it, Iterator end, size_t size, TreeHashIDMap<Hash, EqualTo, InitialCapacity>& inner_table, IndexedHashSet<double>& leaf_table)
{
    /* Base cases */
    if (size == 1)
...
auto insert(const Range& state, TreeHashIDMap<Hash, EqualTo, InitialCapacity>& inner_table, IndexedHashSet<double>& leaf_table)
{
    ...
    if ((static_cast<double>(inner_table.size() + 2 * size) / inner_table.capacity()) > inner_table.max_load_factor())
        inner_table.rehash(leaf_table);

    std::cout << "Insert start" << std::endl;

    const auto result = inner_table.insert_root(Slot<Index>(insert_recursively(state.begin(), state.end(), size, inner_table, leaf_table), size));
...
template<typename Hash, typename EqualTo, size_t InitialCapacity>
inline void
read_state(Index root_index, const TreeHashIDMap<Hash, EqualTo, InitialCapacity>& inner_table, const IndexedHashSet<double>& leaf_table, DoubleList& out_state)
{
    /* Observe: a root slot wraps the root tree_index together with the length that defines the tree structure! */
    const auto& slot = inner_table.lookup_root(root_index);

    read_state(slot.i1, slot.i2, inner_table, leaf_table, out_state);
}
...
public:
    ...
    const_iterator(const TreeHashIDMap<Hash, EqualTo, InitialCapacity>& inner_table, const IndexedHashSet<double>& leaf_table, Index root, bool begin) :
...
template<typename Hash, typename EqualTo, size_t InitialCapacity>
inline auto begin(Index root, const TreeHashIDMap<Hash, EqualTo, InitialCapacity>& inner_table, const IndexedHashSet<double>& leaf_table)
{
...
template<typename Hash, typename EqualTo, size_t InitialCapacity>
inline auto range(Index root, const TreeHashIDMap<Hash, EqualTo, InitialCapacity>& inner_table, const IndexedHashSet<double>& leaf_table)
{
