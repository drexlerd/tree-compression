...
struct TreeSplit {
    std::size_t vars_to_left;  // Number of variables to the left of this split
    std::size_t mid_tree;      // Index of the tree node that is the split point
};
...
struct MergeSchedule {
    ...
    std::vector<TreeSplit> traversal_splits;  // Preorder: 1=merge, 0=leaf
};
...
recursive_tree_dfs(const TreeNode* node)
{
    std::vector<std::size_t> variable_order;
    std::vector<bool> traversal_bits;

    // Local lambda for recursion
    std::function<void(const TreeNode*)> dfs = [&](const TreeNode* cur) {
...
inline std::unique_ptr<TreeNode> compute_merge_tree(const std::vector<std::size_t>& costs) {
    using PQElem = std::unique_ptr<TreeNode>;
    ...
    for (std::size_t i = 0; i < costs.size(); ++i) {
        pq.push(std::make_unique<TreeNode>(TreeNode{costs[i], i, true}));
    }
    ...
    while (pq.size() > 1) {
        ...
        pq.push(std::make_unique<TreeNode>(TreeNode{
            a->cost * b->cost,
            false,
            std::move(a),
            std::move(b)
...
template< std::forward_iterator Iterator>
    ...
    inline std::pair<unsigned long, bool> emplace_recursively(Iterator state_begin, Iterator state_end,
           const std::vector<TreeSplit>& mid_spans,
           ...
           FixedHashSetSlot& table)
{
    ...
    auto insert(const Range& state, const std::vector<TreeSplit>& mid_spans, FixedHashSetSlot& tree_table)
    {
    ...
    inline void read_state_recursively(Index index, size_t size,
        size_t pos, const std::vector<TreeSplit>& mid_spans,
...
template <typename FixedHashSetSlot>
inline void read_state(Index tree_index, const size_t size, const std::vector<TreeSplit>& mid_spans, const FixedHashSetSlot& tree_table, State& out_state)
{
