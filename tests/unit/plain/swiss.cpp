...
TEST(VallaTests, PlainUintHashIDMapTest)
{
    ...
    auto inner_table = IndexedHashSet<Slot>();
...
{
    auto table = IndexedHashSet<Slot>();
    auto leaf_table = IndexedHashSet<double>();
...

    {
        ...
        EXPECT_EQ(s0, IndexList(v::begin(s0_idx, table), v::end()));
    }

    {
        const auto s0 = IndexList {};
        EXPECT_EQ(s0, IndexList(v::begin(EMPTY_ROOT_SLOT, table), v::end()));
    }

    {
        ...
        EXPECT_EQ(s0, DoubleList(v::begin(s0_idx, table, leaf_table), v::end(leaf_table)));
    }

    {
        const auto s0 = DoubleList {};
        EXPECT_EQ(s0, DoubleList(v::begin(EMPTY_ROOT_SLOT, table, leaf_table), v::end(leaf_table)));
    }
}
...
TEST(VallaTests, PlainUintHashIDMapExhaustiveTest)
{
    ...
    std::vector<IndexList> ils;
    ils.reserve(num_sequences);
    std::vector<DoubleList> dls;
    dls.reserve(num_sequences);

    IndexList start_il(sequence_size);
    for (auto& v : start_il)
        v = index_dist(rng);
    ils.push_back(start_il);

    DoubleList start_dl(sequence_size);
    for (auto& v : start_dl)
        v = double_dist(rng);
    ...
    auto out_il = IndexList {};
    auto out_dl = DoubleList {};

    auto irs = std::vector<Slot> {};
    auto drs = std::vector<Slot> {};
    ...
    for (size_t i = 0; i < ils.size(); ++i)
    {
        ...
        out_il.clear();
        out_il.insert(out_il.end(), v::begin(ir, table), v::end());
        EXPECT_EQ(il, out_il);
        ...
        out_dl.clear();
        out_dl.insert(out_dl.end(), v::begin(dr, table, leaf_table), v::end(leaf_table));
        EXPECT_EQ(dl, out_dl);
        ...
        for (size_t j = 0; j <= i; ++j)
        {
            ...
            out_il.clear();
            out_il.insert(out_il.end(), v::begin(ir_2, table), v::end());
            EXPECT_EQ(il_2, out_il);
        ...
        for (size_t j = 0; j <= i; ++j)
        {
            ...
            out_dl.clear();
            out_dl.insert(out_dl.end(), v::begin(dr_2, table, leaf_table), v::end(leaf_table));
            EXPECT_EQ(dl_2, out_dl);
